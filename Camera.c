...
Camera makeCamera(double verticalFOV, double aspectRatio, double focalLength, double aperture, Point3 origin, Point3 target)
{
	...
	Vector3 lowerLeftCorner		= subtractVectors(origin, addVectors(addVectors(halfHorizontal, halfVertical), focalLengthMultW));
...
Ray getRay(Camera *camera, double s, double t)
{
	...
	Point3 rayTarget = addVectors( addVectors( camera->lowerLeftCorner, scaleVector(camera->horizontal, s) ), scaleVector(camera->vertical, t) );
